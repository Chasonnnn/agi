...
from contextshift_deid.constants import ACTION_DIR
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Compose the math action train split with an exported UPChieve train split.")
    parser.add_argument("--math-train-file", type=Path, default=ACTION_DIR / "train.jsonl")
    parser.add_argument("--upchieve-train-file", type=Path, required=True)
    parser.add_argument(
        "--output-file",
        type=Path,
        default=ACTION_DIR / "train_mixed_upchieve_english_social_v1.jsonl",
    )
    ...
    if duplicate_ids:
        ...
        raise SystemExit(
            f"Duplicate ids across train sources: found {len(duplicate_ids)} duplicates. First ids: {preview}"
        )

    _write_jsonl(args.output_file, merged_rows)
    validate_action_records(args.output_file)

    subject_counts = Counter(str(row.get("subject", "unknown")) for row in merged_rows)
    source_subject_counts = {
        "math": Counter(str(row.get("subject", "unknown")) for row in math_rows),
        "upchieve": Counter(str(row.get("subject", "unknown")) for row in upchieve_rows),
    }
    label_counts = Counter(str(row.get("action_label", "unknown")) for row in merged_rows)
    summary = {
        ...
        "counts_by_source_subject": {
            source_name: dict(counter)
            for source_name, counter in source_subject_counts.items()
        },
