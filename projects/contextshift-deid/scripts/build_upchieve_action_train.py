...
DEFAULT_SUBJECT_TOTAL = 300
DEFAULT_SLICE_TOTAL = 150
DEFAULT_PRIVATE_PRIOR_RESERVE = 40
...
def _private_prior_score(item: dict[str, Any]) -> int:
    ...
    score = 0
    if span_text:
        if _span_pattern(GREETING_SPAN_TEMPLATE, span_text).search(text):
            score += 3
        if _span_pattern(SELF_IDENTIFY_TEMPLATE, span_text).search(text):
            score += 3
    if entity_type in {"SCHOOL", "LOCATION"} and SCHOOL_LOCATION_PRIVATE_RE.search(text):
        score += 2
    if CONTACT_CUE_RE.search(text):
        score += 1
    if str(item["entity_type"]) in CONTROL_TAGS:
        score += 1
    return score
...
def _select_natural(
    ...
    *,
    ...
    private_candidates = [
        item
        for item in items
        if _private_prior_score(item) > 1
    ]
    ...
    return selected, {
        ...
        "private_prior_available": len(private_candidates),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the UPChieve English/Social Studies train annotation pool.")
    parser.add_argument(
        "--input-file",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Processed action exports whose dialogue_id values should be excluded from train sampling.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ANNOTATION_DIR / "upchieve_context_train",
    )
    ...
    split_name = "upchieve_english_social_train"
    ...
    output_file = args.output_dir / "action_pool_upchieve_english_social_train.jsonl"
